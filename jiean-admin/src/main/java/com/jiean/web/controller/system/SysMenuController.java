...
public class SysMenuController extends BaseController {
    ...
    public RestResponse getInfo(@PathVariable Long menuId) {
        return RestResponse.success(menuService.selectMenuById(menuId));
    }
    ...
    public RestResponse treeselect(SysMenu menu) {
        ...
        Long userId = loginUser.getUser().getUserId();
        List<SysMenu> menus = menuService.selectMenuList(menu, userId);
        return RestResponse.success(menuService.buildMenuTreeSelect(menus));
    }
    ...
    public RestResponse roleMenuTreeselect(@PathVariable("roleId") Long roleId) {
        LoginUser loginUser = tokenService.getLoginUser(ServletUtils.getRequest());
        List<SysMenu> menus = menuService.selectMenuList(loginUser.getUser().getUserId());
        RestResponse restResponse = RestResponse.success();
        restResponse.put("checkedKeys", menuService.selectMenuListByRoleId(roleId));
        restResponse.put("menus", menuService.buildMenuTreeSelect(menus));
        return restResponse;
    }
    ...
    public RestResponse add(@Validated @RequestBody SysMenu menu) {
        if (UserConstants.NOT_UNIQUE.equals(menuService.checkMenuNameUnique(menu))) {
            return RestResponse.error("新增菜单'" + menu.getMenuName() + "'失败，菜单名称已存在");
        } else if (UserConstants.YES_FRAME.equals(menu.getIsFrame())
                && !StringUtils.startsWithAny(menu.getPath(), Constants.HTTP, Constants.HTTPS)) {
            return RestResponse.error("新增菜单'" + menu.getMenuName() + "'失败，地址必须以http(s)://开头");
        }
    ...
    public RestResponse edit(@Validated @RequestBody SysMenu menu) {
        if (UserConstants.NOT_UNIQUE.equals(menuService.checkMenuNameUnique(menu))) {
            return RestResponse.error("修改菜单'" + menu.getMenuName() + "'失败，菜单名称已存在");
        } else if (UserConstants.YES_FRAME.equals(menu.getIsFrame())
                && !StringUtils.startsWithAny(menu.getPath(), Constants.HTTP, Constants.HTTPS)) {
            return RestResponse.error("新增菜单'" + menu.getMenuName() + "'失败，地址必须以http(s)://开头");
        } else if (menu.getMenuId().equals(menu.getParentId())) {
            return RestResponse.error("新增菜单'" + menu.getMenuName() + "'失败，上级菜单不能选择自己");
        }
    ...
    public RestResponse remove(@PathVariable("menuId") Long menuId) {
        if (menuService.hasChildByMenuId(menuId)) {
            return RestResponse.error("存在子菜单,不允许删除");
        }
        if (menuService.checkMenuExistRole(menuId)) {
            return RestResponse.error("菜单已分配,不允许删除");
        }
